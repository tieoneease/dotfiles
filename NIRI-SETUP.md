...
Plugins are loaded by PluginService on startup. Enable/disable via Settings > Plugins
or by editing `plugins.json`. Keep-awake uses Noctalia's built-in KeepAwake widget
(CC toggle, `--what=idle`).
...
/etc/systemd/logind.conf.d/10-power-and-lid.conf     # power button = suspend (all laptops, sudo)
/etc/systemd/logind.conf.d/20-suspend-then-hibernate.conf  # lid/power → suspend-then-hibernate (sam-ganymede only)
/etc/systemd/sleep.conf.d/10-suspend-then-hibernate.conf   # HibernateDelaySec=15min (sam-ganymede only)
...
│           └── i18n/en.json
│       └── screen-toggle/                           # eDP-2 toggle (Zenbook Duo only)
├── systemd/user/
...
### Trigger chain — power button (suspend)

```
Physical key: Power button
  → logind HandlePowerKey=suspend (PowerKeyIgnoreInhibited=yes)
    → Zenbook Duo: systemctl suspend (s2idle)
    → GPD Win Max 2: systemctl suspend-then-hibernate (s2idle → hibernate after 15min)
```
...
**See:** `POWER-MANAGEMENT.md` for the full diagnosis, forensic evidence, and real-world
behavior of every power state.

### Architecture

All suspend/hibernate is handled by standard systemd components — no custom scripts or
plugins. The stack is intentionally minimal:

```
logind (lid close / power button)
  → systemctl suspend or suspend-then-hibernate
    → kernel s2idle (processes frozen, RAM powered)
    → [after HibernateDelaySec] systemctl hibernate (RAM → disk, power off)

swayidle (idle timeouts only)
  → 5 min idle → Noctalia lock screen
  → 10 min idle → DPMS off

Noctalia lockOnSuspend=true
  → locks screen before any suspend (replaces swayidle before-sleep)

Noctalia built-in KeepAwake
  → --what=idle, blocks swayidle (screen stays on, no auto-lock)
  → does NOT block lid-close suspend (correct — you want suspend when closing lid)
```

### Power button

**Config:** `/etc/systemd/logind.conf.d/10-power-and-lid.conf`

logind handles the power button directly — niri's power-key handling is disabled
(`disable-power-key-handling` in config.kdl). This avoids the niri re-suspend bug
([#2233](https://github.com/niri-wm/niri/issues/2233)) where niri sees the
wake-from-suspend power key event and immediately re-suspends.

```ini
HandlePowerKey=suspend           # suspend instead of poweroff
PowerKeyIgnoreInhibited=yes      # always works, even with KeepAwake active
```

### Idle & lock (swayidle)

**Spawned by:** niri `spawn-sh-at-startup` in config.kdl
...
|-------|--------|
| 5 min idle | Noctalia lock screen |
| 10 min idle | Power off monitors |
| Any input after DPMS off | Power on monitors |

Lock-before-suspend is handled by Noctalia's `lockOnSuspend` setting (not swayidle).

### Suspend-then-hibernate (GPD Win Max 2)

**Config:** `/etc/systemd/logind.conf.d/20-suspend-then-hibernate.conf` +
`/etc/systemd/sleep.conf.d/10-suspend-then-hibernate.conf`

AMD Phoenix S0ix is broken — the CPU never enters deep idle during s2idle, so the machine
drains battery at near-active rates (~5-15W) while "suspended". To prevent this:

1. Lid close or power button → s2idle for 15 minutes (quick resume if opened soon)
2. After 15 min → systemd auto-hibernates to disk (0W, indefinite standby)
3. Resume from hibernate: power button → BIOS POST → kernel loads RAM from swap (~15s)

**Requirements (deployed by arch_setup.sh):**
- 16GB btrfs swap file at `/swap/swapfile`
- Kernel params: `resume=UUID=<root> resume_offset=<offset>` (via Limine drop-in)
- `HibernateDelaySec=15min` in sleep.conf

### Behavior matrix

| Scenario | Zenbook Duo (sam-duomoon) | GPD Win Max 2 (sam-ganymede) |
|----------|--------------------------|------------------------------|
| **Lid close** | s2idle suspend | s2idle → hibernate after 15min |
| **Lid open (quick)** | Resume ~1-3s, lock screen | Resume ~1-3s, lock screen |
| **Lid open (after 15min)** | Resume ~1-3s (still s2idle) | Resume ~15s from hibernate |
| **Power button** | Suspend (always, ignores inhibitors) | Suspend-then-hibernate |
| **5 min idle** | Lock screen | Lock screen |
| **10 min idle** | Monitors off (DPMS) | Monitors off (DPMS) |
| **KeepAwake ON** | Screen stays on, no auto-lock | Screen stays on, no auto-lock |
| **KeepAwake ON + lid close** | Still suspends (KeepAwake is --what=idle) | Still suspend-then-hibernates |
