...
/etc/systemd/logind.conf.d/10-power-and-lid.conf     # power button + lid switch config (sudo)
...
│   ├── lid-display-handler.sh                       # lid close/open → display power management
...
Physical key: Power button
  → logind HandlePowerKey=suspend (PowerKeyIgnoreInhibited=yes)
    → systemctl suspend (s2idle, always works even with sleep-inhibitor active)
```
...
**Sleep mode:** s2idle (Low-power S0 idle) — the kernel/firmware default on both the Zenbook Duo and GPD Win Max 2.

**⚠ S3 deep sleep is broken on the Zenbook Duo.** Forcing `MemorySleepMode=deep` causes failed resume on lid close (hard restart required). Do not create a sleep.conf.d override.

### Power button

**Config:** `/etc/systemd/logind.conf.d/10-power-and-lid.conf`

logind handles the power button directly — niri's power-key handling is disabled (`disable-power-key-handling` in config.kdl). This avoids the niri re-suspend bug ([#2233](https://github.com/niri-wm/niri/issues/2233)) where niri sees the wake-from-suspend power key event and immediately re-suspends.

```ini
HandlePowerKey=suspend           # suspend instead of poweroff
PowerKeyIgnoreInhibited=yes      # always works, even with sleep-inhibitor active
```
...
| Before suspend | Lock screen (prevents desktop flash on resume) |

### Sleep inhibitor (keep-awake)

**Plugin:** `noctalia/.config/noctalia/plugins/sleep-inhibitor/`
**Toggle:** CC widget (coffee icon) or `qs -c noctalia-shell ipc call plugin:sleep-inhibitor toggle`

Uses `systemd-inhibit --what=sleep --mode=block` to prevent suspend while allowing screen blanking (swayidle timeout still works). Replaces the built-in Noctalia KeepAwake which uses `--what=idle` and blocks screen-off.

### Lid handling

**Config:** `LidSwitchIgnoreInhibited=no` in `/etc/systemd/logind.conf.d/10-power-and-lid.conf`

Default logind ignores all inhibitors on lid close (`yes`). With `no`, the sleep-inhibitor plugin can actually block lid-close suspend — the system stays running with the lid closed.

**Lid display handler:** `~/.config/niri/lid-display-handler.sh` (spawned at niri startup)

Monitors logind's `LidClosed` property via D-Bus. When the lid closes with the sleep inhibitor active, it powers off monitors via niri (saves battery under the closed lid). On lid open, it always powers monitors back on.

### Behavior matrix

| Scenario | What happens |
|----------|-------------|
| Lid close, inhibitor OFF | Normal s2idle suspend via logind |
| Lid close, inhibitor ON | Suspend blocked, monitors powered off, system stays running |
| Lid open (after suspend) | Resume + monitors powered on |
| Lid open (after inhibited close) | Monitors powered on |
| Power button pressed | s2idle suspend (always, ignores inhibitors) |
| 5 min idle | Noctalia lock screen |
| 10 min idle | Monitors off via DPMS |
| Before any suspend | Noctalia lock screen |
