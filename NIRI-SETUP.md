...
/etc/systemd/logind.conf.d/10-lid-inhibitor.conf     # LidSwitchIgnoreInhibited=no (sudo)
...
│   ├── force-suspend.sh                             # power button → disable inhibitor + suspend
...
### Trigger chain — power button (force suspend)

```
Physical key: Power button
  → niri keybind: XF86PowerOff → spawn-sh force-suspend.sh
    → force-suspend.sh disables sleep-inhibitor plugin via IPC
      → waits 300ms for inhibitor process to exit
        → systemctl suspend (s2idle)
```
...
**⚠ S3 deep sleep is broken on the Zenbook Duo.** Forcing `MemorySleepMode=deep` causes failed resume on lid close (hard restart required). The `etc/systemd/sleep.conf.d/10-deep-sleep.conf` is kept for reference but is **never deployed**. `arch_setup.sh` actively removes it if found.
...
**Config:** `/etc/systemd/logind.conf.d/10-lid-inhibitor.conf`
...
**Script:** `~/.config/niri/lid-display-handler.sh` (spawned at niri startup)
...
### Power button (force suspend)

**Script:** `~/.config/niri/force-suspend.sh` (bound to `XF86PowerOff`)

Escape hatch for "I'm packing up" — disables the sleep inhibitor via IPC, then triggers `systemctl suspend` (s2idle). Always works regardless of keep-awake state. Works from lock screen (`allow-when-locked=true`).
...
| Power button pressed | Inhibitor disabled → s2idle suspend (always) |
| swayidle 300s timeout | Monitors off via DPMS (unrelated to suspend) |
