...
Kitty sets `TERM=xterm-kitty`, which most remote hosts don't have in their terminfo database. This causes broken terminal behavior (e.g., doubled characters) when SSH-ing directly. The `ssh` alias in `aliases.zsh` automatically uses `kitten ssh` when running inside Kitty, which copies the terminfo to the remote host on first connect.

For hosts that need the terminfo installed manually:

```bash
infocmp -a xterm-kitty | ssh <host> 'mkdir -p ~/.terminfo && tic -x -o ~/.terminfo -'
```
