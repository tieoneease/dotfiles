...
- **Dev workflow extension:** Phase commands (`/idea`, `/research`, `/poc`, `/spec`, `/breakdown`, `/build`) are extension commands in `extensions/dev-workflow/`, not standalone prompt templates. Each command deterministically injects THREAD.md + primary artifact + file manifest into the agent message — no reliance on the agent reading the right files. `/ship` remains a standalone prompt template.
