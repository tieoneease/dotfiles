...
- **Host matching:** Uses `header_regexp` to match any subdomain depth (e.g., `booking-agent.pydantic-agent.sambot.localhost`). Caddy's wildcard site addresses only match a single label, so regex is required for Portless's `<name>.<project>` format.
- **Adding a VPS:** (1) Add SSH host with `LocalForward <port> localhost:1355`, (2) add a `@name` matcher and `handle` block to the Caddyfile, (3) copy config and reload (`sudo systemctl reload caddy`).
...
- **Dev workflow extension:** Phase commands (`/discover`, `/spec`, `/breakdown`, `/build`) are extension commands in `extensions/dev-workflow/`, not standalone prompt templates. `/discover` is the unified entry point replacing the old `/idea`, `/research`, `/poc` — it combines conversational idea refinement with inline research/POC dispatch in one fluid session. Each command deterministically injects THREAD.md + primary artifact + file manifest into the agent message — no reliance on the agent reading the right files. `/ship` remains a standalone prompt template.
...
- **Dev server extension:** `dev_server` tool for non-blocking dev server lifecycle (start/status/logs/stop/alias). Names are auto-namespaced by project (detected from package.json / git / cwd) — agent uses simple names like `name="api"` and gets `http://api.<project>.localhost:1355` via Portless subdomain routing. State at `/tmp/pi-dev-server/<name>.<project>/` (pid, url, status, output.log) — readable by any process. Full [Portless](https://github.com/vercel-labs/portless) integration: auto-starts proxy, alias action for external services (Docker, databases), unified service map (pi-tracked + Portless routes grouped by project) in status, and active service URLs auto-injected into LLM context via `before_agent_start` for cross-service coordination. Shutdown only cleans up current project's servers. Falls back to `localhost:<port>` when Portless is not installed.
