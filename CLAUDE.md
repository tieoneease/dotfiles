...
- **sambot VM:** `Host sambot` with `LocalForward 1355` — auto-forwards Portless so all remote dev servers are accessible locally at `<name>.localhost:1355` when SSH'd in via Tailscale.
- **peachy-data:** `Host peachy-data` with `LocalForward 1356` — same Portless forwarding on port 1356 so it can run alongside sambot without conflicts.
...
- **Subagent names:** Use `worker` (not `code`) for general-purpose coding/implementation tasks. Available agents: `scout` (recon), `planner` (plan), `worker` (implement), `reviewer` (review), `validator` (verify), `researcher` (extract notes), `research-lead` (multi-source research), `poc-lead` (POC execution). There is no agent named `code`.
