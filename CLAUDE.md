...
- **Subagent names:** Use `worker` (not `code`) for general-purpose coding/implementation tasks. Available agents: `scout` (fast Sonnet recon — use liberally for codebase mapping), `worker` (implement), `experimenter` (POC, fast/destructive in worktrees), `reviewer` (end-of-build code review), `validator` (verify), `researcher` (extract notes), `research-lead` (multi-source research), `poc-lead` (POC experiment orchestration). There is no agent named `code` or `planner`.
